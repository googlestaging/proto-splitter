...
==============================================================================*/
#include "third_party/proto_splitter/cc/repeated_field_splitter.h"
...
#include "third_party/proto_splitter/cc/size_splitter.h"
...
RepeatedFieldSplitters<ParentMessage, RepeatedMessage>::Create(
    ::proto2::Message* message, ComposableSplitter* parent_splitter,
  ...
  ASSIGN_OR_RETURN(FieldResult field_ret, GetField(*message, {repeated_field}));
...
absl::StatusOr<int> RepeatedFieldSplitters<
  ...
  ASSIGN_OR_RETURN(auto ret, GetMutableField(message(), {repeated_field_}));
  ...
  for (int i = 0; i < repeated_field_size; ++i) {
    ...
    for (auto factory : *splitter_factories_) {
      ...
      if (new_splitter != nullptr) {
        ASSIGN_OR_RETURN(auto size_diff, new_splitter->BuildChunksReturnSize());
        node_size -= size_diff;
      }
    }
  ...
  if (repeated_msg_split.size() > 1) {
    ...
    for (int i = 1; i < repeated_msg_split.size(); ++i) {
      ...
      RETURN_IF_ERROR(AddChunk(std::move(x), &empty_fields));

      // Move nodes into new_msg.
      ASSIGN_OR_RETURN(auto new_ret,
