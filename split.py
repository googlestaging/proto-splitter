...
from proto_splitter import version as version_lib
from proto_splitter import versions_pb2
  ...
  def write(self, file_prefix: str) -> str:
    ...
    if not chunked_message.chunked_fields:
      ...
      epath.Path(path).write_bytes(
          self._proto.SerializeToString(deterministic=True))
      logging.info("Unchunked file exported to %s", path)
      return path

    path = f"{file_prefix}.cpb"
    with riegeli.RecordWriter(io.FileIO(path, "wb")) as f:
