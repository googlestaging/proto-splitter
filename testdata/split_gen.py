...
from google3.third_party.proto_splitter import chunk_pb2
from google3.third_party.proto_splitter import split
from google3.third_party.proto_splitter import util
from google3.third_party.proto_splitter.testdata import test_message_pb2
...
class DFStringNodeSplitter(StringNodeSplitter):
  ...
  def build_chunks(self) -> Sequence[Union[message.Message, bytes]]:
    ...
    if self._parent_splitter is None:
      ...
      path = os.path.join(SPLITTER_TESTDATA_PATH.value, "df-split-tree.pbtxt")
      epath.Path(path).write_bytes(
          self._chunked_message.SerializeToString(deterministic=True))
...
class BFStringNodeSplitter(StringNodeSplitter):
  ...
  def build_chunks(self) -> Sequence[Union[message.Message, bytes]]:
    ...
    if self._parent_splitter is None:
      self._chunks.append(self._chunked_message)
      path = os.path.join(SPLITTER_TESTDATA_PATH.value, "bf-split-tree.pbtxt")
      epath.Path(path).write_bytes(
          self._chunked_message.SerializeToString(deterministic=True))
...
def main(argv: Sequence[str]) -> None:
  ...
  path = os.path.join(SPLITTER_TESTDATA_PATH.value, "split-tree.pbtxt")
  epath.Path(path).write_bytes(
      string_tree.SerializeToString(deterministic=True))
