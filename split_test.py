...
from proto_splitter import chunk_pb2
from proto_splitter import split
...
class SplitRepeatedStringTest(absltest.TestCase):
  ...
  def testWrite(self):
    ...
    with riegeli.RecordReader(open(f"{path}.cpb", "rb")) as reader:
      ...
      for expected_index, expected_data, chunk in zip(
        ...
        self.assertLen(expected_data, chunk_info.size)
...
class NoOpSplitterTest(absltest.TestCase):
  ...
  def testWriteNoChunks(self):
    ...
    self.assertEqual(len(proto.strings), len(parsed_proto.strings))
    for s1, s2 in zip(proto.strings, parsed_proto.strings):
