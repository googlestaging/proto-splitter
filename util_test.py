...
from proto_splitter import util
from proto_splitter.testdata import test_message_pb2
...
class UtilTest(absltest.TestCase):
  ...
  def test_get_field_and_desc(self):
    ...
    # compare repeated field manually, absl_test has no proto-comparing utility
    for f1, f2 in zip(proto.field_one.repeated_field, field):
      self.assertEqual(f1.string_field, f2.string_field)
      self.assertEqual(len(f1.map_field_uint32.items()),
                       len(f2.map_field_uint32.items()))
      for k, v in f1.map_field_uint32.items():
        self.assertEqual(v, f2.map_field_uint32[k])

    field, field_desc = util.get_field(proto, ["field_one", 2, 1])
    self.assertIsInstance(field, test_message_pb2.ManyFields)
    self.assertEqual("repeated_field", field_desc.name)
    self.assertEqual(2, field_desc.number)
    # compare string and map fields manually
    self.assertEqual(proto.field_one.repeated_field[1].string_field,
                     field.string_field)
    for k, v in proto.field_one.repeated_field[1].map_field_uint32.items():
      self.assertEqual(v, field.map_field_uint32[k])
